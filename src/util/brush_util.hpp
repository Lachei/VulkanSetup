...
inline structures::dynamic_struct<gpu_brush, float> create_gpu_brush_data(const range_brush_refs& range_brushes, const lasso_brush_refs& lasso_brushes){
    ...
    for(int brush: size_range(axis_brushes)){
        gpu_data[brush] = static_cast<float>(cur_offset);                       // brush_offset
        gpu_data[cur_offset++] = static_cast<float>(axis_brushes[brush].size());// nAxisMaps
        ...
        for(const auto& [axis, ranges]: axis_brushes[brush]){
            gpu_data[axis_offsets++] = static_cast<float>(cur_offset);          // axisOffsets
            gpu_data[cur_offset++] = static_cast<float>(ranges.size());         // nRanges
            gpu_data[cur_offset++] = static_cast<float>(axis);                  // axis
    ...
    for(int i: size_range(lasso_brushes)){
        const auto& lasso_brush = *lasso_brushes[i];
        gpu_data[gpu_data->lasso_brush_offset + i] = static_cast<float>(cur_offset);// brush_offset
        gpu_data[cur_offset++] = static_cast<float>(lasso_brush.size());            // nPolygons
        uint32_t polygon_offsets = cur_offset;
        cur_offset += static_cast<uint32_t>(lasso_brush.size());                    // skipping the polygon_offsets
        for(const auto& polygon: lasso_brush){
            gpu_data[polygon_offsets++] = static_cast<float>(cur_offset);           // polygon_offsets
            gpu_data[cur_offset++] = static_cast<float>(polygon.attr1);             // attr1
            gpu_data[cur_offset++] = static_cast<float>(polygon.attr2);             // attr2
            gpu_data[cur_offset++] = static_cast<float>(polygon.borderPoints.size());// nBorderPoints
